...
```{r Загрузка данных}
dtrend <- gtrends(c("data scientist", "data science", "R programming"), time = "all")
```
...
get_top10_regions <- function(x){
top10_regions <- head(interest_by_region %>% 
                                    filter(keyword==x) %>% 
                                    group_by(location) %>%
                                    summarise(hits=sum(hits)) %>%
                                    arrange(desc(hits)), 10) # поиск 10 регионов с наивысшей популярностью запроса
...
get_top10_regions("data scientist")
get_top10_regions("data science")
get_top10_regions("R programming")
